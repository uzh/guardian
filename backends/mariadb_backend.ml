...
module Make(R : Guardian.Role_s)(CONFIG : sig val connection_string : string end) () = struct
  ...
  include Guardian.Make_persistence(struct
    ...
    let get_roles id : (Guardian.Role_set.t, string) Lwt_result.t =
      ...
      | Ok s -> Lwt.return(Guardian.Role_set.of_yojson(Yojson.Safe.from_string s))
      | Error err -> Lwt.return_error(Caqti_error.show err)

    let get_perms target_spec : (Guardian.Authorizer.auth_rule list, string) Lwt_result.t =
      let%lwt res =
