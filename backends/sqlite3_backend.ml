module Make (R : Guardian.Role_s) = struct
  ...
  module Backend = struct
    ...
    let get_roles id =
      ...
      let stmt =
        Sqlite3.prepare db "SELECT roles FROM guardian_entities WHERE id = ?"
      in
      let _ =
        let open Sqlite3 in
    ...
    let get_owner id =
      let id' = Uuidm.to_string id in
      let stmt =
        Sqlite3.prepare db "SELECT parent FROM guardian_entities WHERE id = ?"
      in
      let* () =
        let open Sqlite3 in
    ...
    let mem_authorizable id =
      let stmt =
        Sqlite3.prepare db "SELECT * FROM guardian_entities WHERE id = ?"
      in
      let _ =
        let open Sqlite3 in
