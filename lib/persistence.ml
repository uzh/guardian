module type Backend_store_s = sig
  ...
  val create_authorizable :
    id:Uuidm.t -> ?owner:Uuidm.t -> role_set -> (unit, string) monad
...
module type S = sig
  ...
  val get_authorizable :
    typ:'kind -> Uuidm.t -> ('kind authorizable, string) Lwt_result.t

  val put_perms :
    auth_rule list -> (auth_rule list, auth_rule list) Lwt_result.t
