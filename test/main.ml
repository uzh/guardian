...
module Tests (Backend : Ocauth.Persistence_s) = struct
  ...
  let test_check_roles _ () =
    (let* roles = Backend.get_roles (snd aron) in
     ...
     if diff = [] then Lwt.return_ok ()
       ...
       Lwt.return_error
         (Printf.sprintf "Got %s for roles of authorizable %s, but expected %s."
            received
  ...
  let test_revoke_roles _ () =
    (let* () =
       Backend.grant_roles (snd aron)
         (Ocauth.Role_set.singleton (`Editor Uuidm.nil))
     in
     let* () =
       let* roles = Backend.get_roles (snd aron) in
       if Ocauth.Role_set.mem (`Editor Uuidm.nil) roles then Lwt.return_ok ()
       else
         Lwt.return_error
           "Didn't successfully add the role we intended to remove."
     in
     let* () =
       Backend.revoke_roles (snd aron)
         (Ocauth.Role_set.singleton (`Editor Uuidm.nil))
     in
     let* roles = Backend.get_roles (snd aron) in
  ...
  let test_read_perms _ () =
    ...
    >|= Alcotest.(check (result unit string))
          "Read the global permissions we've just pushed." (Ok ())
  ...
  let test_drop_perms _ () =
    (let* () = Backend.put_perm bad_perm in
     ...
     let* () =
       match perms with
       | [ perm ] ->
           if perm = bad_perm then Lwt.return_ok ()
           else
             Lwt.return_error
  ...
  let test_update_owned _ () =
    ...
    >|= Alcotest.(check (result bool string))
          "Chris can update an article owned by Chris." (Ok true)
  ...
  let test_admin_update_others' _ () =
    (let%lwt aron_ent = User.to_authorizable aron in
     ...
     | Error err -> raise (Failure err))
    >|= Alcotest.(check bool)
          "Aron (admin) can update an article Chris owns" true
  ...
  let can_update_self _ () =
    (let%lwt chris_article' = Article.to_authorizable chris_article in
     ...
     | Ok chris_article' ->
         let%lwt res =
           Article.update_title chris_article' chris_article "Updated Title"
         in
         Lwt.return (Result.is_ok res)
     | Error err -> raise (Failure err))
  ...
  let article_cannot_update_other_article _ () =
    (let%lwt chris_article' = Article.to_authorizable chris_article in
     ...
     | Ok chris_article' ->
         let%lwt res =
           Article.update_title chris_article' aron_article "Updated Title"
         in
         Lwt.return (Result.is_error res)
     | Error err -> raise (Failure err))
  ...
  let set_owner _ () =
    ...
    >|= Alcotest.(check (result bool string))
          "Article cannot update another article." (Ok true)
  ...
  let make_test_cases (module Backend : Ocauth.Persistence_s) name =
    ...
    [
      ( Printf.sprintf "(%s) Managing authorizables." name,
        [
          Alcotest_lwt.test_case "Create an authorizable." `Quick
            T.test_create_authorizable;
          Alcotest_lwt.test_case "Retrieve an authorizable." `Quick
      ...
      ( Printf.sprintf "(%s) Admins should be able to do everything." name,
        [
          Alcotest_lwt.test_case "Update someone else's article." `Quick
            T.test_admin_update_others';
        ] );
      ( Printf.sprintf
          ...
          name,
        [
          Alcotest_lwt.test_case "Update own article." `Quick
            T.test_update_owned;
        ] );
      ( Printf.sprintf
          "(%s) An authorizable should be able to do everything to itself." name,
      ...
      ( Printf.sprintf
        ...
        [
          ...
          Alcotest_lwt.test_case "Cannot update" `Quick
            T.article_cannot_update_other_article
